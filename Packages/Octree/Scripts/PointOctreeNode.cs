...
//     Copyright (c) 2024, Makar Osokin, http://elmortem.com
//     All rights reserved.
// </copyright>
...
namespace NetOctree
{
    public partial class PointOctree<T>
    {
        ...
        private class Node
        {
            ...
            /// <summary>
            /// For reverting the bounds extends after temporary changes
            /// </summary>
            private Vector3 _actualBoundsExtents;
            ...
            public void GetChildBounds(List<Bounds> bounds)
            {
                if (HasChildren)
                {
            ...
            public Node(float baseLengthVal, float minSizeVal, float3 centerVal)
            {
            ...
            public void GetNearby(ref Ray ray, float maxDistance, List<T> result)
            {
                ...
                _bounds.extents = _actualBoundsExtents;
                if (!intersected)
                {
                    return;
                }
            ...
            public void GetNearby(ref float3 position, float maxDistance, List<T> result)
            {
                ...
                {
                    var o = _objects[i];
                    if (o.Pos.Equals(position))
                        continue;
                    
                    if (math.lengthsq(position - o.Pos) <= sqMaxDistance) //TODO need to check faster
                    {
                        result.Add(o.Obj);
                    }
                }
            ...
            public bool IsColliding(ref float3 position, float maxDistance, bool ignoreSelf = true)
            {
                // Does the node contain this position at all?
                // Note: Expanding the bounds is not exactly the same as a real distance check, but it's fast.
                // TODO: Does someone have a fast AND accurate formula to do this check?
                ...
                if (_objects.Count > 0)
                {
                    ...
                    {
                        var o = _objects[i];
                        if (ignoreSelf && o.Pos.Equals(position))
                            continue;
                ...
                if (_children != null)
                {
                    var index = BestFitChild(position);
                    var count = 0;
                    while(count < 8)
                    {
                        if (_children[index].IsColliding(ref position, maxDistance, ignoreSelf))
                            return true;
            ...
            public bool IsColliding(ref T obj, ref float3 position, float maxDistance)
            {
                // Does the node contain this position at all?
                // Note: Expanding the bounds is not exactly the same as a real distance check, but it's fast.
                // TODO: Does someone have a fast AND accurate formula to do this check?
                ...
                if (_objects.Count > 0)
                {
                    ...
                    for (int i = 0; i < _objects.Count; i++)
                    {
                        ...
                        if (ReferenceEquals(o.Obj, obj) || o.Obj.Equals(obj))
                            continue;
                        
                        if (math.lengthsq(position - o.Pos) <= sqrMaxDistance) //TODO need to check faster
                        {
                            return true;
                        }
                    }
                ...
                if (_children != null)
                {
                    ...
                    while(count < 8)
                    {
                        if (_children[index].IsColliding(ref obj, ref position, maxDistance))
                            return true;
            ...
            private void SetValues(float baseLengthVal, float minSizeVal, Vector3 centerVal)
            {
                ...
                _bounds = new Bounds(Center, new Vector3(SideLength , SideLength, SideLength));
                _actualBoundsExtents = _bounds.extents;
                ...
                _childBounds[0] = new Bounds(Center + new float3(-quarter, quarter, -quarter), childActualSize);
                _childBounds[1] = new Bounds(Center + new float3(quarter, quarter, -quarter), childActualSize);
                _childBounds[2] = new Bounds(Center + new float3(-quarter, quarter, quarter), childActualSize);
                _childBounds[3] = new Bounds(Center + new float3(quarter, quarter, quarter), childActualSize);
                _childBounds[4] = new Bounds(Center + new float3(-quarter, -quarter, -quarter), childActualSize);
                _childBounds[5] = new Bounds(Center + new float3(quarter, -quarter, -quarter), childActualSize);
                _childBounds[6] = new Bounds(Center + new float3(-quarter, -quarter, quarter), childActualSize);
                _childBounds[7] = new Bounds(Center + new float3(quarter, -quarter, quarter), childActualSize);
            }
            ...
            private void Split()
            {
                ...
                _children[0] = new Node(newLength, _minSize, Center + new float3(-quarter, quarter, -quarter));
                _children[1] = new Node(newLength, _minSize, Center + new float3(quarter, quarter, -quarter));
                _children[2] = new Node(newLength, _minSize, Center + new float3(-quarter, quarter, quarter));
                _children[3] = new Node(newLength, _minSize, Center + new float3(quarter, quarter, quarter));
                _children[4] = new Node(newLength, _minSize, Center + new float3(-quarter, -quarter, -quarter));
                _children[5] = new Node(newLength, _minSize, Center + new float3(quarter, -quarter, -quarter));
                _children[6] = new Node(newLength, _minSize, Center + new float3(-quarter, -quarter, quarter));
                _children[7] = new Node(newLength, _minSize, Center + new float3(quarter, -quarter, quarter));
            }
            ...
            private static bool Encapsulates(Bounds outerBounds, float3 point)
            {
