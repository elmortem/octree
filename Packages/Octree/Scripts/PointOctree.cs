...
namespace NetOctree
{
    ...
    public partial class PointOctree<T>
    {
        ...
        public Bounds MaxBounds => new Bounds(_rootNode.Center,
            new Vector3(_rootNode.SideLength, _rootNode.SideLength, _rootNode.SideLength));
        ...
        /// <summary>
        /// Check if the specified point intersect with anything in the tree. See also: GetNearby.
        /// </summary>
        /// <param name="position">The position. Passing as ref to improve performance since it won't have to be copied.</param>
        /// <param name="maxDistance">Maximum distance from the position to consider.</param>
        /// <returns>True if there was a collision.</returns>
        public bool IsColliding(float3 position, float maxDistance)
        {
            return _rootNode.IsColliding(ref position, maxDistance);
        }
        
        /// <summary>
        /// Check if the specified point intersect with anything in the tree. See also: GetNearby.
        /// </summary>
        /// <param name="obj">Object for exclude from search.</param>
        /// <param name="position">The position. Passing as ref to improve performance since it won't have to be copied.</param>
        /// <param name="maxDistance">Maximum distance from the position to consider.</param>
        /// <returns>True if there was a collision.</returns>
        public bool IsColliding(T obj, float3 position, float maxDistance)
        {
            return _rootNode.IsColliding(ref obj, ref position, maxDistance);
        }
        ...
        private void Grow(float3 direction)
        {
            ...
            float3 newCenter = _rootNode.Center + new float3(xDirection * half, yDirection * half, zDirection * half);
